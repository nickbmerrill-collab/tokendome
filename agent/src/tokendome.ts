...
type Upstream = {
  base: string;
  // When true, events through this upstream are tagged is_local=true (cost=0).
  // Lets you point e.g. `openai` at a local vLLM/llama.cpp/MLX server and
  // have it correctly attributed as local compute on the leaderboard.
  is_local?: boolean;
};
...
function loadConfig(): Config {
  ...
  const merged: Config = { ...def, ...saved, upstreams: { ...def.upstreams } };
  if (saved.upstreams) {
    for (const k of Object.keys(saved.upstreams) as (keyof Config['upstreams'])[]) {
...
function estimateCostUSD(e: Event): number {
  if (e.is_local) return 0;
  const key = `${e.provider}:${e.model.toLowerCase()}`;
  let best: { in: number; out: number; cr?: number } | null = null;
  let bestLen = -1;
  for (const [k, v] of Object.entries(PRICES)) {
    if (key.startsWith(k) && k.length > bestLen) { best = v; bestLen = k.length; }
  }
  if (!best) return 0;
...
async function flushLoop(cfg: Config) {
  while (true) {
    await new Promise(r => setTimeout(r, 3000));
    if (queue.length === 0) continue;
    const batch = queue.splice(0, queue.length);
    // Offline mode: no cloud reporting, just drain the queue.
    if (!cfg.agent_token || !cfg.user_id) continue;
    try { await report(cfg, batch); }
    catch (err: any) {
...
async function report(cfg: Config, events: Event[]) {
...
// ─── Upstream routing ───────────────────────────────────────────────────────
//
// We look at the request URL path to decide who to talk to.
//   /v1/chat/completions       → OpenAI-compat (OpenAI, or Ollama if model matches)
//   /v1/messages               → Anthropic native
//   /v1beta/models/*:generate* → Google Gemini
//   /api/generate, /api/chat   → Ollama native
//
// For OpenAI-compat we also sniff the model name: models starting with
// `claude-` route to Anthropic (via its OpenAI-compat endpoint), `gemini-`
// to Google, `ollama/` prefix strips and routes to local Ollama, and plain
// names default to OpenAI.
...
// ─── Token extractors (one per response shape) ──────────────────────────────

type ExtractCtx = {
  provider: string;
  is_local: boolean;
  contentType: string;
  bodyChunks: Buffer[];   // non-streaming
  sseEvents: string[];    // streaming: full event strings, already split
  model: string;          // from request body
};

async function extractOpenAI(ctx: ExtractCtx) {
  ...
  // SSE stream
  let lastUsage: any = null, model = ctx.model;
  ...
  for (const ev of ctx.sseEvents) {
    ...
    try {
      ...
      if (type === 'message_start' && j.message) {
        ...
        if (typeof j.usage.output_tokens === 'number') output = j.usage.output_tokens;
        if (typeof j.usage.input_tokens === 'number') input = j.usage.input_tokens;
      }
    } catch {}
  }
  if (input || output) {
...
async function extractOllamaNative(ctx: ExtractCtx) {
  ...
  for (const ln of lines) {
    try {
      ...
      if (j.model) model = j.model;
      if (typeof j.prompt_eval_count === 'number') input = j.prompt_eval_count;
      if (typeof j.eval_count === 'number') output = j.eval_count;
    } catch {}
  }
  if (input || output) {
    pushEvent({
...
async function extractGoogle(ctx: ExtractCtx) {
  ...
  if (ctx.contentType.includes('application/json')) {
    try { tryExtract(JSON.parse(Buffer.concat(ctx.bodyChunks).toString('utf8'))); } catch {}
    return;
  }
  for (const ev of ctx.sseEvents) {
    const dataLine = ev.split('\n').find(l => l.startsWith('data: '));
    if (!dataLine) continue;
    try { tryExtract(JSON.parse(dataLine.slice(6))); } catch {}
  }
}

// ─── HTTP proxy ─────────────────────────────────────────────────────────────

function startProxy(cfg: Config) {
  const server = http.createServer((clientReq, clientRes) => {
    ...
    clientReq.on('error', (e: any) => console.error('client err', e.message));
  });
...
function handleRequest(cfg: Config, req: http.IncomingMessage, res: http.ServerResponse, body: Buffer) {
  ...
  const upReq = lib.request({
    ...
    let reqModel = '';
    try { reqModel = (JSON.parse(finalBody.toString('utf8')).model || '').toString(); } catch {}
    ...
    if (isStream) {
      ...
      upRes.on('data', (chunk: Buffer) => {
        ...
        if (contentType.includes('text/event-stream')) {
          ...
          while ((idx = carry.indexOf('\n\n')) !== -1) {
            events.push(carry.slice(0, idx)); carry = carry.slice(idx + 2);
          }
        } else {
          // NDJSON: treat each line as one "event"
          let idx;
          while ((idx = carry.indexOf('\n')) !== -1) {
            events.push(carry.slice(0, idx)); carry = carry.slice(idx + 1);
          }
        }
      });
      upRes.on('end', () => {
      ...
      const bufs: Buffer[] = [];
      upRes.on('data', (c: Buffer) => { bufs.push(c); res.write(c); });
      upRes.on('end', () => {
        res.end();
        if ((upRes.statusCode || 0) < 300) {
          r.extractor({
...
async function login(token: string, serverUrl?: string) {
  ...
  if (dot < 1) { console.error('bad token format'); process.exit(1); }
  const uid = Number(token.slice(0, dot));
  const secret = token.slice(dot + 1);
  if (!uid || !secret) { console.error('bad token'); process.exit(1); }
  cfg.user_id = uid;
...
function importConfigCmd(blob: string) {
  if (!blob) { console.error('usage: tokendome import-config <base64-blob>'); process.exit(1); }
  let parsed: any;
  try {
...
async function doctorCmd() {
  ...
  const bad = (m: string, hint?: string) => { console.log('✘', m); if (hint) console.log('  →', hint); };

  // 1. Config sane
  if (cfg.user_id && cfg.agent_token) ok(`config:   logged in as user ${cfg.user_id} → ${cfg.server_url}`);
  else                                bad('config:   not logged in', 'tokendome login <token> ' + cfg.server_url);
  ...
  try {
    ...
    if (serverOk) ok(`server:   reachable (${cfg.server_url})`);
    else          bad(`server:   unexpected status ${r.status}`, 'check ' + cfg.server_url + ' in a browser');
  } catch (e: any) {
  ...
  try {
    ...
    agentOk = r.ok;
    if (agentOk) ok(`agent:    listening on :${cfg.port}`);
    else         bad(`agent:    health endpoint returned ${r.status}`);
  } catch {
  ...
  if (cfg.user_id && cfg.agent_token && serverOk) {
    try {
      ...
      if (r.ok) ok('ingest:   accepted a signed test event (1 in / 1 out, marked local)');
      else      bad(`ingest:   server rejected (${r.status}) — ${(await r.text().catch(() => '')).slice(0, 120)}`, "check that your token isn't stale and your clock is in sync");
    } catch (e: any) {
  ...
  if (process.platform === 'darwin' || process.platform === 'linux') {
    ...
    if (platform === 'launchd') {
      ...
      if (r.code === 0) ok('service:  launchd unit loaded — auto-starts on every login');
      else              console.log('·  service:  not installed (run: tokendome service install) — only relevant if you want auto-start');
    } else {
      const r = execv('systemctl', ['--user', 'is-active', 'tokendome.service']);
      if (r.code === 0) ok('service:  systemd user unit active');
      else              console.log('·  service:  not installed (run: tokendome service install)');
    }
  }
...
function platformService(): 'launchd' | 'systemd' | null {
  if (process.platform === 'darwin') return 'launchd';
  if (process.platform === 'linux') return 'systemd';
  return null;
}

function execv(cmd: string, args: string[]): { code: number | null; stdout: string; stderr: string } {
...
function resolveAll(host: string): string[] {
...
async function captureAllInstall() {
  ...
  for (const h of CAPTURE_HOSTS) console.log('    -', h);
  console.log('');
  ...
  if (platform === 'darwin') {
    ...
    const ips: string[] = [];
    for (const h of CAPTURE_HOSTS) for (const ip of resolveAll(h)) ips.push(ip);
    if (ips.length === 0) {
    ...
    if (r1.code !== 0) { console.error('✘ pfctl load failed:', r1.stderr.trim()); process.exit(1); }
    const r2 = execv('sudo', ['pfctl', '-E']);
    if (r2.code !== 0 && !/pf already enabled/i.test(r2.stderr)) {
      console.error('✘ pfctl enable failed:', r2.stderr.trim()); process.exit(1);
    }
...
function captureAllUninstall() {
  ...
  if (platform === 'darwin') {
    ...
    if (fs.existsSync(PF_RULES_PATH)) fs.unlinkSync(PF_RULES_PATH);
...
function setKey(dotPath: string, value: string) {
  const cfg: any = loadConfig();
...
function main() {
  ...
  switch (cmd) {
    ...
    case 'service': {
      ...
      console.error('usage: tokendome service install|uninstall|status'); process.exit(1);
    }
    case 'capture-all': {
      const sub = rest[0];
      if (sub === 'install')   return void captureAllInstall();
      if (sub === 'uninstall') return captureAllUninstall();
      console.error('usage: tokendome capture-all install|uninstall'); process.exit(1);
    }
