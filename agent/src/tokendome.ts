...
type Config = {
  server_url: string;      // e.g. https://tokendome.example.com
  user_id: number;
  agent_token: string;
  port: number;
  // Upstream routing. model prefix → upstream config.
  upstreams: {
    openai:    { base: string };   // https://api.openai.com
    anthropic: { base: string };   // https://api.anthropic.com
    google:    { base: string };   // https://generativelanguage.googleapis.com
    ollama:    { base: string };   // http://localhost:11434
  };
};
...
function status() {
  ...
  for (const [k, v] of Object.entries(cfg.upstreams)) console.log(`  ${k}: ${v.base}`);
}
